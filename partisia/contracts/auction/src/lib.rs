...
/// Custon struct for TokenClaims used by the contracts claim-map.
///
/// ### Fields:
///
/// * `tokens_for_bidding`: [`u64`], The claimable tokens for bidding.
///
/// * `tokens_for_sale`: [`u64`], The claimable tokens for sale.
#[derive(ReadWriteRPC, ReadWriteState, CreateTypeSpec, Clone, PartialEq, Debug)]
...
fn token_contract_transfer() -> Shortname {
    Shortname::from_be_bytes(&[202, 237, 213, 191, 2]).unwrap()
}
...
///
/// * `token_amount_for_sale`: [`u64`], the amount of tokens for sale.
///
...
///
/// * `reserve_price`: [`u64`], the reserve price (minimum cost of the tokens for sale).
///
/// * `min_increment`: [`u64`], the minimum increment of each bid.
///
...
/// Initial function to bootstrab the contracts state.
///
/// ### Parameters:
///
/// * `ctx`: [`ContractContext`], initial context.
///
/// * `token_amount_for_sale`: [`u64`], the amount of tokens that is going to be sold.
///
...
pub fn start_callback(
    ...
    if ctx.sender != new_state.contract_owner {
        panic!("Start callback can only be called by the creator of the contract through start");
    }
    if new_state.status != CREATION {
        panic!("start callback should only be called while the contract is being created");
    }
...
/// * `state`: [`AuctionContractState`], the current state of the contract.
///
/// * `bid_amount`: [`u64`], the amount of tokens in the bid.
///
/// ### Returns
///
/// The unchanged state object of type [`AuctionContractState`].
